...
function readJson(req) {
	return new Promise((resolve, reject) => {
		...
		req.on("data", (chunk) => {
			size += chunk.length;
			if (size > 16384) reject(/* @__PURE__ */ new Error("body-too-large"));
			else chunks.push(chunk);
		});
		req.on("end", () => {
			try {
		...
		handler: async (req, res) => {
			...
			try {
				...
				if ("sessionTtlHours" in input) {
					if (!Number.isInteger(input.sessionTtlHours) || input.sessionTtlHours < 1 || input.sessionTtlHours > 720) throw new Error("invalid-session-ttl");
					patch.sessionTtlHours = input.sessionTtlHours;
				}
